    ...
    for( var i=0; i < 4; i++)
        for( var j=0; j<4; j++){
            ...
            if(board[i][j]==0){
                ...
                // theNumberCell.css('line-hight', cellSideLength);
...
function moveLeft(){
    ...
    for( var i=0; i < 4; i++)
        for( var j=1; j<4; j++){
            if(board[i][j] != 0){ //如果该位置有数字，则判断其左侧所有位置 
                for( var k=0; k<j; k++){
                    // 左边没有数字且中间没有数字阻挡
...
function moveUp(){
    ...
    //moveup
    // 对整个棋盘进行遍历
    for( var j=0; j < 4; j++) //cloumn
        for( var i=1; i<4; i++){ //row
            if(board[i][j] != 0){ //如果该位置有数字，则判断其上方所有位置 
                for( var k=0; k<i; k++){
                    // 上边没有数字且中间没有数字阻挡
